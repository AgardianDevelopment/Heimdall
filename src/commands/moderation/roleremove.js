...
class RoleRemoveCommand extends Command {
  ...
  async exec (msg, { member, role }) {
    // Fetch guild role and member, send response if not found else remove role
    ...
    if (Object.entries(logChan).length === 0) return msg.util.reply(`${member.tag} has had role removed.`).then(msg.delete({ timeout: 5000 }))
